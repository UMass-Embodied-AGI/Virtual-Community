...
def process_frame_agents(frame_idx, args, names_order, name_to_color, agent_cam_images, global_images, demo_folder, last_text_actions):
    ...
    for i, name in enumerate(names_order):
        ...
        pos = avatar_positions[i]
        text_clips.append(
            TextClip(font=os.path.join(get_assets_dir(), "OpenSans-Regular.ttf"), text=name, font_size=20, color=(name_color_bgr_255[name][0], name_color_bgr_255[name][1], name_color_bgr_255[name][2]))
    ...
    for i, name in enumerate(selected_agents):
        ...
        # text_clips.append(
        #     TextClip(font=os.path.join(get_assets_dir(), "OpenSans-Regular.ttf"), text=wrapped_text_upper[name], font_size=15, color=(255, 255, 255))
        #     .with_position((pos[0], pos[1] - 50))
        #     .with_duration(1/args.fps)
        # )
        text_clips.append(
            TextClip(font=os.path.join(get_assets_dir(), "OpenSans-Regular.ttf"), text=wrapped_text_bottom[name], font_size=15, color=(255, 255, 255))
            .with_position((pos[0], pos[1] + 350))
            .with_duration(1/args.fps)
        )
    
    text_clips.append(
        TextClip(font=os.path.join(get_assets_dir(), "OpenSans-Regular.ttf"), text="Time: " + str(current_time), font_size=50, color=(255, 255, 255))
