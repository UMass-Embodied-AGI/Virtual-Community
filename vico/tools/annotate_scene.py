...
from vico.tools.constants import google_map_type_to_coarse, google_map_coarse_to_types, coarse_types_priority, ASSETS_PATH
...
def filter_places(args):
	...
	filter_names = ["animal"]
	with open(f"{ASSETS_PATH}/scenes/{args.scene}/raw/places_enu_original.json", 'r') as file:
		json_text = file.read()
	...
	with open(f"{ASSETS_PATH}/scenes/{args.scene}/raw/places_enu.json", 'w') as json_file:
		json.dump(filtered_places, json_file, indent=4)
	# if args.remove_temp:
	#     os.remove(f"{ASSETS_PATH}/scenes/{args.scene}/raw/places_enu_original.json")

def save_metadata(args):
	with open(f"{ASSETS_PATH}/scenes/{args.scene}/raw/places_enu.json", 'r') as file:
		json_text = file.read()
	json_text_cleaned = re.sub(r',\s*([}\]])', r'\1', json_text) # remove trailing commas before closing braces or brackets
	places = json.loads(json_text_cleaned)
	...
	with open(f"{ASSETS_PATH}/scenes/{args.scene}/raw/places_full.json", 'w') as json_file:
		json.dump(metadata, json_file, indent=4)
	if args.remove_temp:
		os.remove(f"{ASSETS_PATH}/scenes/{args.scene}/raw/places_enu.json")
...
def overlay_locations_desp_on_image():
	...
	for building in building_metadata:
		for i, place in enumerate(building_metadata[building]["places"]):
			...
			draw.text((pixel_x, pixel_y + i * 14), indicator_text, font=ImageFont.truetype(f"{ASSETS_PATH}/arial.ttf", 20), fill=color)
	...
	annotated_image.save(f"{ASSETS_PATH}/scenes/{args.scene}/global_annotated.png")
...
def get_building_to_places():
	...
	json.dump(inaccessible_buildings, open(f"{ASSETS_PATH}/scenes/{args.scene}/raw/inaccessible_buildings.json", 'w'), indent=4)
	...
	json.dump(place_metadata, open(f"{ASSETS_PATH}/scenes/{args.scene}/place_metadata.json", 'w'), indent=4)
	building_metadata = update_buildings(buildings_with_capped_places, place_metadata)
	json.dump(building_metadata, open(f"{ASSETS_PATH}/scenes/{args.scene}/building_metadata.json", 'w'), indent=4)
	json.dump(building_to_places, open(f"{ASSETS_PATH}/scenes/{args.scene}/raw/building_to_places.json", 'w'), indent=4)
	json.dump(mismatched, open(f"{ASSETS_PATH}/scenes/{args.scene}/raw/mismatched.json", 'w'), indent=4)
	# sort type_stats by key
	type_stats = dict(sorted(type_stats.items(), key=lambda item: item[0]))
	json.dump(type_stats, open(f"{ASSETS_PATH}/scenes/{args.scene}/raw/type_stats.json", 'w'), indent=4)
	json.dump(generate_new_type_stats(building_metadata), open(f"{ASSETS_PATH}/scenes/{args.scene}/raw/type_stats_accessible.json", 'w'))

	# if args.align_legacy:
	# 	legacy_place_path = f"{ASSETS_PATH}/scenes/{args.scene}/legacy/place_metadata.json"
...
if __name__ == '__main__':
	...
	if not args.only_update_bbox:
		...
		# if os.path.exists(f"{ASSETS_PATH}/scenes/{args.scene}/orthographic_scale_800.png"):
		...
		if not os.path.exists(os.path.join(ASSETS_PATH, "scenes", args.scene, "global.png")):
			...
			# Image.fromarray(global_depth).save(os.path.join(f"{ASSETS_PATH}/scenes/{args.scene}/global_depth.png"))
		...
		if not args.search_original_places and os.path.exists(os.path.join(ASSETS_PATH, "scenes", args.scene, "raw", "places_full.json")):
			...
			# shutil.copy2(f"{ASSETS_PATH}/scenes/{args.scene}/raw/places_full.json", f"{ASSETS_PATH}/scenes/{args.scene}/raw/places_full_old.json")
			# with open(f"{ASSETS_PATH}/scenes/{args.scene}/raw/places_full.json", 'r') as file:
			...
			# with open(f"{ASSETS_PATH}/scenes/{args.scene}/raw/places_full.json", 'w') as file:
	...
	# if not os.path.exists(f"{ASSETS_PATH}/scenes/{args.scene}/building_metadata.json"):
